...
func (t *LLRB[K, V]) Graphviz() string {
	...
	if t.root != nil {
		if t.root.isRed() {
			fmt.Fprintf(buf, "%v [color=red fontcolor=red]\n", t.root.key)
		} else {
			fmt.Fprintf(buf, "%v [color=black fontcolor=black]\n", t.root.key)
		}
	}
	fmt.Fprintf(buf, "}\n")
	return buf.String()
...
func (nd *LLRBNode[K, V]) String() string {
	return fmt.Sprintf("%v:%v", nd.key, nd.value)
}
...
func (nd *LLRBNode[K, V]) graphviz(w io.Writer) {
	...
	if nd.left != nil {
		fmt.Fprintf(w, `%v -> %v`, nd.key, nd.left.key)
		if nd.left.isRed() {
	...
	if nd.right != nil {
		fmt.Fprintf(w, `%v -> %v`, nd.key, nd.right.key)
		if nd.right.isRed() {
			fmt.Fprintf(w, ` [color=red]`)
		}
		fmt.Fprintln(w)
	...
	if nd.isRed() {
		fmt.Fprintf(w, "%v [color=red fontcolor=red]\n", nd.key)
	}
}
